...
foreach ($nuspec in $nuspecs)
{
    ...
    # This works if we are following the usual convention of creating the nuspec file in the root
    # of the project and giving it the same name as the project.
    ...
    # Get the version from the built assembly. We want the "pre" bit from AIV, but get rid of the
    # reset of the message which includes commit shas etc.
    $ver = dnv --read $asm --what aiv
    $ver = $ver.substring(0, $ver.indexof(','))
    
    # Pack and drop directly into the $dropDir folder.
